        ...
        <div class="layui-fluid">
            <div class="layui-row layui-col-space15">
                ...
                <div class="layui-form-item">
					...
					<div id="contractNumDiv" style="display:none" class="layui-inline">
					...
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input type="text" name="percentage" id="percentage" lay-verify="number"  autocomplete="off" class="layui-input">
						</div>
					</div>
				</div>
                ...
                <div class="layui-form-item">
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input id="balance" type="text" name="balance"  lay-verify="required|number" placeholder="&yen;"  autocomplete="off" class="layui-input">
						</div>
					</div>
					...
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input type="text" name="interest" id="interest" lay-verify="required|number"  autocomplete="off" class="layui-input">
						</div>
					</div>
                ...
                <fieldset class="layui-elem-field layui-field-title " style="border-color: #009688;">
			  		<legend>共借人信息</legend>
              		 ...
              		 <div class="layui-card-body">
		                <button type="button" id="addCommon" class="layui-btn"  style="float:right; margin-right: 5%">添加共借人</button>
	                	<table class="layui-table" id="conLoanerTable" lay-filter="loanerFilter"></table>
	                </div>
				...
				<fieldset class="layui-elem-field layui-field-title " style="border-color: #009688;">
              		...
              		<div class="layui-card-body">
		                <button type="button" id="addMortProperty" class="layui-btn"  style="float:right; margin-right: 5%">添加抵押房</button>
                ...
                <fieldset class="layui-elem-field layui-field-title " style="border-color: #009688;">
		  			<legend>备用房信息</legend>
              		 ...
              		 <div class="layui-card-body">
		                <button type="button" id="addBackupProperty" class="layui-btn"  style="float:right; margin-right: 5%">添加备用房</button>
	                	<table class="layui-table" id="backupProperty" lay-filter="propertyFilter"></table>
	                </div>
                ...
                <fieldset class="layui-elem-field layui-field-title " style="border-color: #009688;">
                    ...
                    <div class="layui-form-item" id="subDiv" style="margin-left: 15%;">
                        <div class="layui-input-block">
                            <button id="update" lay-submit="" lay-filter="suu" class="lafite-layui-btn2 lafite_margin_left_20px">提交</button>
                            <button id="tempUpdate" name="tempUpdate" lay-submit="" lay-filter="tempSuu" class="lafite-layui-btn2 lafite_margin_left_20px">临时保存</button>
                        </div>
                    </div>
...
<!--     	<a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a> -->
	...
	//					alert(document.getElementById("statusFlag").value);
			...
			$.ajax({
			       ...
			       success:function(data){
				       ...
				       $('#contractId').val(data);//通过id给标签赋值
				       $('#statusFlag').val("未提交");//通过id给标签赋值
				       	/* var aaa = document.getElementById("contractId");
				       	alert(JSON.stringify(aaa.value));  */
			       }
			  });
		} 
	    ...
	    //共同借款人表渲染
	    window.renderLoaner('#conLoanerTable','common');
	    
	    //添加借款人
		$(document).on('click','#addLoaner',function(){
			openAddLoanPage('loanerTable','添加借款人','loaner','conLoanerTable');
		});
   		//添加共同借款人
   		$(document).on('click','#addCommon',function(){
 	  		openAddLoanPage('conLoanerTable','添加共借人','common','loanerTable');
		});
	    ...
	    //备用房表渲染
	    window.renderProperty('#backupProperty','backup');
	    
	    //添加抵押房
		$(document).on('click','#addMortProperty',function(){
			openAddProertyPage('mortgageProperty','添加抵押房信息','mort','backupProperty');
		});
   		//添加备用房
   		$(document).on('click','#addBackupProperty',function(){
   			openAddProertyPage('backupProperty','添加备用房信息','backup','mortgageProperty');
		});
		...
		form.on('submit(tempSuu)', function(data){
            ...
            /* 	var balance = document.getElementById("balance");
            	balance.removeAttribute("lay-verify"); */
	...
	//打开添加借款人界面的方法
	function openAddLoanPage(tableName,titleName,loanerType,checkTableName){
			...
			var index = layer.open({
	            ...
	            yes: function(index,btn1){
					...
					if(arr.data.length != 0){
						...
						var cacheArraCheck = layui.table.cache[checkTableName];
		 		        for(var k in cacheArraCheck){
		 		        	if(cacheArraCheck.length == 0){
		 		        		closeFlag = true;
		 		        		break;
		 		        	}else  if(arr.data[0].loanerId == cacheArraCheck[k].loanerId){
								layer.msg(JSON.stringify(arr.data[0].citizenID+'不能同时为借款人和共借人！'),{time:3000});
			 		        	closeFlag = true;
			 		        	break;
		 		        	}
		 		       	}
	...
	//打开添加房产信息界面的方法
	function openAddProertyPage(tableName,titleName,loanerType,checkTableName){
		 layui.use(['table'],function() {
			    var $ = layui.jquery,table = layui.table;
			...
			var index = layer.open({
	            type:2, 	
	            title: titleName,
	            content: '<%=basePath%>pawn/toPropertyList',   
	            area: ['80%', '100%'],
	            btn: ['选择'],
	            shadeClose: true,
	            yes: function(index,btn1){
					...
					if(arr.data.length != 0){
		                ...
		                var closeFlag = false;
		 		        ...
		 		        for(var k in cacheArra){
		 		        	if(cacheArra.length == 0){
		 		        		closeFlag = true;
		 		        		break;
		 		        	}else  if(arr.data[0].propertyId == cacheArra[k].propertyId){
			 		        	
								layer.msg(JSON.stringify('该房产已添加！'),{time:3000});
		 		       ...
		 		       var cacheArraCheck = layui.table.cache[checkTableName];
		 		        for(var k in cacheArraCheck){
		 		        	if(cacheArraCheck.length == 0){
		 		        		closeFlag = true;
		 		        		break;
		 		        	}else  if(arr.data[0].propertyId == cacheArraCheck[k].propertyId){
								layer.msg(JSON.stringify('该房产已添加为抵押房或备用房！'),{time:3000});
			 		        	closeFlag = true;
			 		        	break;
		 		        	}
		 		       	}
						...
						if(!closeFlag){
							//新增房产绑定信息
			                bindProperty(arr.data[0].propertyId,loanerType,tableName);
						}
					}
	...
	//房产信息表拿到的房产跟合同绑定并重载
	function bindProperty(propertyIdNum,mortgageTp,tableNam){
		layui.use(['table'],function() {
		    var $ = layui.jquery,table = layui.table;
	        ...
	        table.on('tool(propertyFilter)', function(obj){
	            ...
	            if(obj.event === 'detail'){
	               layer.open({
	                   ...
	                   success: function (layero, index) {
	                   		var body = layer.getChildFrame('body', index);  //巧妙的地方在这里哦
	                   		body.find("#propertySub").css('display','none');
	                   		body.find("#propertyRe").css('display','none');
	    	               // layer.alert('编辑行：<br>'+ JSON.stringify(data));

	    	                // 循环所有键
	    	                for(var key in propertyLine) {
		    	                body.find("#"+key).val(propertyLine[key]);
	    	                }
