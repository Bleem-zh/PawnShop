							    ...
							    <!-- <div class="layui-inline" >
								    <label class="layui-form-label">更新原因</label>
									    <div class="layui-input-block layui-input-inline">
									      <select name="updateReason" id="updateReason" lay-verify="required"></select>
									    </div>
								</div> -->
...
<!--     	<a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a> -->
	...
	layui.use(['form', 'upload','table','layer','element','dropdown','util'],function() {
	    var $ = layui.jquery
	    	,upload = layui.upload
	    	,form = layui.form 
	    	,table = layui.table
	...
	//					alert(document.getElementById("statusFlag").value);
	    ...
	    //共同借款人表渲染
	    window.renderLoaner('#conLoanerTable','common','conLoanDiv');
	    ...
	    //备用房表渲染
	    window.renderProperty('#backupProperty','backup','backupDiv');
	...
	function initStatusSelect(){
		...
		/* layui.use(['form'],function() {
		    ...
		    if(beforeStatus === '待放款'){
		        str+="<option value='已放款'>已放款</option>";
		    }else if(beforeStatus === '待还款'){
		        str+="<option value='按计划还款'>按计划还款</option>";
		        //str+="<option value='已逾期'>已逾期</option>";
		        
		        str+="<optgroup label='已逾期'>";
		        str+="<option value='M1'>M1</option>";
		        str+="<option value='M2'>M2</option>";
		        str+="<option value='M3'>M3</option>";
		        str+="<option value='M3+'>M3+</option></optgroup>";
		        
		        str+="<option value='已赎回'>已赎回</option>";
		        
		    }else if(beforeStatus === '已逾期'){
		        str+="<option value='逾期处理完成'>逾期处理完成</option>";
		        str+="<option value='无偿还能力'>无偿还能力</option>";
		    }
		    
		  //jquery赋值方式
	        $("#updateReason").html(str);
	        //重新渲染生效
	        form.render('select');
		}); */ 
	}
	...
	//房产的渲染
	function renderProperty(tableId,mortgageTypeStr,divName){
	    var conNum = document.getElementById("contractId")
	    //alert(conNum.value);
	    ...
	    layui.use(['table','layer'],function() {
	        ...
	        var abcTable = table.render({
	            elem:tableId
	            ,url:'<%=basePath%>pawn/getApproveProduct'
	            ,where:{contractId: conNum.value,
